...
# Read the CSV file
data <- read.csv("Prevalence of Asthma - Summarized Sheet (combined countries).csv", stringsAsFactors = FALSE)

# Histogram 1: Proportion of moderate to hazardous days per state
ggplot(data, aes(x = `Proportion.of.moderate.to.hazardous.days`)) +
  geom_histogram(bins = 10, fill = "skyblue", color = "black") +
  labs(title = "Proportion of Moderate to Hazardous Days per State",
       x = "Proportion of Moderate to Hazardous Days",
       y = "Number of States") +
  theme_minimal()

# Histogram 2: Average proportion per state
ggplot(data, aes(x = `Average.proportion.per.state`)) +
  geom_histogram(bins = 10, fill = "lightcoral", color = "black") +
  labs(title = "Average Proportion of Poor Air Quality per State",
       x = "Average Proportion per State",
       y = "Number of States") +
  theme_minimal()
```
